...
| Advisories | `cargo audit` | **pass** (1 advisory, suppressed with a reason — see F-11) |
...
| F-09 | Low | dead reservation | `src/main.rs:2734`, `src/prompts.rs:52` | `logout` is reserved as a slash-command name in two places, but no `/logout` handler exists (only `goshcoder auth logout`, `provider_cli.rs:160`). | Confirmed against the binary: `/logout` → "unknown command". Not documented in the README, so it is an over-reservation rather than a broken promise. | open |
| F-10 | Low | GUI conformance | `src/ui.rs:17-29` vs `src/main.rs:399` | The fullscreen TUI hardcodes 24-bit `Color::Rgb` unconditionally. `color_enabled()` honours `NO_COLOR`, but only the line/`run` renderers consult it; `ui.rs` never does. | A user who sets `NO_COLOR` (which the project already respects elsewhere) still gets a fully coloured alternate-screen UI. Inconsistent with the project's own convention. | open |
| F-11 | Low | GUI conformance | `src/ui.rs:29,273` | `FAINT` `#3E5760` on `BACKGROUND` `#0A0A0A` is a **2.58:1** contrast ratio (computed per WCAG 2.x relative luminance), used for the status-bar keyboard hints and the unfilled progress track. | Below WCAG AA 4.5:1 for normal text and below even the 3:1 large-text floor. `MUTED` (5.7:1) and `TEXT` (~16:1) are fine. | open |
| F-12 | Low | consistency | `src/ui.rs:17-29` and `src/markdown.rs:17-22` | The colour palette (`ACCENT`, `CYAN`, `TEXT`, `MUTED`, `FAINT`) is declared twice, once per module, with no shared token source. | Two sources of truth for design tokens; they can drift silently. | open |
...
| F-14 | Low | docs | `src/sessions.rs` | `sessions gc` is implemented and reachable but appears in neither the README's `Use` section nor `goshcoder --help`. | Implemented behaviour that is undocumented (the reverse direction Phase 3 asks for). | open |
...
_(none yet — awaiting the go-ahead on the order above)_
...
Phases 0–2 and 4 are complete for the whole tree. Phase 1 and Phase 3 are
complete for every module listed under "Read line by line" in Phase 0, which
covers every security boundary in the program (filesystem confinement,
subprocess execution, all credential storage and resolution, every loopback
HTTP server, archive extraction, HTML export, terminal-escape handling,
SigV4, the SSE/stream bounds, and the provider request path).
...
for the defect classes in this audit's scope (redirect/TLS policy, credential
handling, unbounded reads, path handling, spawn/lock patterns) and exercised
through the binary, but have **not** had a full line-by-line pass. Roughly 35k
of 57k production lines got the targeted rather than exhaustive treatment.
Say the word and I will finish those at full depth; I am flagging it rather
than letting "Phase 1 complete" imply more coverage than it has.
