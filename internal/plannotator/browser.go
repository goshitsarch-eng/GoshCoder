...
// original Plannotator plan surface while keeping the implementation
...
// Changes view, matching Plannotator's resubmission workflow.
...
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Plannotator</title><style>
...
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{{.Title}} · Plannotator</title><style>
...
$('#toggle-panel').onclick=()=>rightPanel.classList.toggle('open');$('#theme').onclick=()=>{document.documentElement.classList.toggle('light');localStorage.setItem('plannotator-theme',document.documentElement.classList.contains('light')?'light':'dark')};if(localStorage.getItem('plannotator-theme')==='light')document.documentElement.classList.add('light');render();
})();
</script></body></html>`))
