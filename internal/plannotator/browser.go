...
func prepareReviewDocument(markdown string) ([]reviewLine, []reviewHeading) {
	...
	for index, text := range parts {
		...
		line := reviewLine{Number: index + 1, Text: text, Display: text, Kind: "paragraph", Indent: reviewIndent(text)}
...
func reviewIndent(text string) int {
	count := 0
	for _, r := range text {
		if r == ' ' {
			count++
...
<article class="document" id="document">{{range .Lines}}<div id="line-{{.Number}}" class="line {{.Kind}}" style="--indent:{{.Indent}}" data-line="{{.Number}}" data-text="{{.Text}}" data-display="{{.Display}}"><span class="ln">{{.Number}}</span>{{.Display}}</div>{{end}}</article>
